...
def test_ocr_verification_draft_submit_approve(http_client):
    creator = register_user(http_client, role="admin")
    approver = register_user(
        http_client,
        role="admin",
        factory_name=creator["factory_name"],
        company_code=creator["company_code"],
    )
    creator_headers = {"Cookie": f"auth_session={creator['session_token']}"}
    approver_headers = {"Cookie": f"auth_session={approver['session_token']}"}

    created = http_client.post("/ocr/verifications", data=_verification_form(), headers=creator_headers)
    ...
    listing = http_client.get("/ocr/verifications", headers=creator_headers)
    ...
    assert approved.json()["approved_by"] == approver["user_id"]
...
def test_ocr_verification_reject_requires_reason(http_client):
    ...
    creator_headers = {"Cookie": f"auth_session={creator['session_token']}"}
    reviewer_headers = {"Cookie": f"auth_session={reviewer['session_token']}"}
...
def test_operator_can_submit_own_ocr_verification_but_cannot_approve(http_client):
    ...
    operator_headers = {"Cookie": f"auth_session={operator['session_token']}"}
...
def test_ocr_verification_export_uses_reviewed_rows(http_client):
    ...
    created = http_client.post("/ocr/verifications", data=_verification_form())
    assert created.status_code == HTTPStatus.CREATED, created.text
    verification = created.json()

    exported = http_client.get(f"/ocr/verifications/{verification['id']}/export")
    ...
    # Row 2 is the draft warning row (merged across columns)
    assert "DRAFT" in str(sheet["A2"].value or "")
    # Row 3+ is data
    assert sheet["A3"].value == "2026-03-29"
    assert str(sheet["B3"].value) == "125"
    assert str(sheet["C3"].value) == "125 / ok"
...
def test_ocr_verification_create_persists_source_image_and_ai_routing_meta(http_client):
    ...
    source = http_client.get(payload["source_image_url"])
...
def test_approved_ocr_verification_export_is_marked_trusted(http_client):
    ...
    exported = http_client.get(f"/ocr/verifications/{verification_id}/export", headers=creator_headers)
    assert exported.status_code == HTTPStatus.OK, exported.text
...
def test_ocr_verification_summary_uses_only_approved_documents_as_trusted(http_client):
    ...
    summary = http_client.get("/ocr/verifications/summary", headers=creator_headers)
