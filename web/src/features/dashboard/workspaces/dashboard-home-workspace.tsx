...
// Lazy-load the operator branch. Managers and owners are the main consumers
// of /dashboard, so the operator JSX is not in their critical bundle. The
// operator chunk loads only when an operator (or worker-mode role) lands here.
const OperatorDashboardWorkspace = dynamic(
  () =>
    import("./operator-dashboard-workspace").then(
      (mod) => mod.OperatorDashboardWorkspace,
    ),
  { ssr: false },
);
  ...
  if (isOperatorHome) {
    return (
      <OperatorDashboardWorkspace
        ...
        syncing={syncing}
        alerts={workerAlerts}
        todayShiftCards={todayShiftCards}
        locale={locale}
