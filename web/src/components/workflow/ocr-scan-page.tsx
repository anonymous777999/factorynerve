                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                setStatusTone("warning");
                                try {
                                  const record = await persistStructuredDraft();
                                  const id = record?.id ?? savedId;
                                  if (!id) {
                                    setStatus("Could not save draft before sending.");
                                    setStatusTone("error");
                                    return;
                                  }
                                  setStatus("Sending document for review...");
                                  await submitOcrVerification(id, "");
                                  setStatus("Document sent for supervisor review.");
                                  setStatusTone("success");
                                  signalWorkflowRefresh("ocr-scan-submitted");
                                } catch (err) {
                                  setStatus(formatApiErrorMessage(err, "Could not send for review."));
                                  setStatusTone("error");
                                }
                              }}
                            >
