          ...
          {reviewIssues.length ? (
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                disabled={!criticalCount || criticalCount === resolvedIssueKeys.filter((key) => reviewIssues.find((i) => i.key === key)?.tone === "critical").length}
                onClick={() => {
                  reviewIssues
                    .filter((issue) => issue.tone === "critical" && !resolvedIssueKeys.includes(issue.key))
                    .forEach((issue) => onMarkIssueChecked(issue.key));
                  const next = reviewIssues.find((issue) => !resolvedIssueKeys.includes(issue.key));
                  if (next) onSelectIssue(next.key, "issues");
                }}
              >
                ✓ Check all critical
              ...
              <Button
                variant="outline"
                className="px-3 py-2 text-xs"
                disabled={!warningCount || warningCount === reviewIssues.filter((i) => i.tone === "warning").length - resolvedIssueKeys.filter((key) => reviewIssues.find((i) => i.key === key)?.tone === "warning").length}
                onClick={() => {
                  reviewIssues
                    .filter((issue) => issue.tone === "warning" && !resolvedIssueKeys.includes(issue.key))
                    .forEach((issue) => onMarkIssueChecked(issue.key));
                  const next = reviewIssues.find((issue) => !resolvedIssueKeys.includes(issue.key));
                  if (next) onSelectIssue(next.key, "issues");
                }}
...
export default function OcrVerificationPage() {
  ...
  useEffect(() => {
    if (dirtyRowIndexes.length === 0) return;
  ...
  const approveNeedsOverride = unresolvedCriticalCount > 0 && (reviewerNotes.trim().length < 40 || criticalResolvedCount < 3);
  ...
  // Tab-through keyboard navigation — Alt+Down for next issue, Alt+Up for previous
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.altKey && e.key === "ArrowUp") {
        ...
        const prev = reviewIssues
          .slice()
          .reverse()
          .find((issue) => !resolvedIssueKeys.includes(issue.key) && issue.key !== selectedIssueKey);
        if (prev) {
