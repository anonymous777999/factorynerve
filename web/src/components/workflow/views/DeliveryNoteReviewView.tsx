...
function getDeliveryNoteSections(data: OcrPreviewResult) {
  ...
  headers.forEach((header, index) => {
    ...
    } else if (/item|product|description|qty|quantity|rate|amount|price|hsn|sac|unit|uom|ordered|delivered/i.test(h)) {
...
export function DeliveryNoteReviewView({
  ...
  const tabs = [
    ...
    { id: "items", label: "Items", count: sections.items.rows.length },
    { id: "vehicle", label: "Vehicle", count: sections.vehicle.rows.length },
  ...
  return (
    <Card className={cn("border-[var(--border-strong)]", className)}>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium">Delivery Note Review</CardTitle>
      </CardHeader>
