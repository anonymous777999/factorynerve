...
function getPOSections(data: OcrPreviewResult) {
  ...
  headers.forEach((header, index) => {
    ...
    } else if (/item|product|description|qty|quantity|rate|amount|price|hsn|sac|unit|uom|spec/i.test(h)) {
...
export function PurchaseOrderReviewView({
  ...
  const tabs = [
    ...
    { id: "terms", label: "Terms & Conditions", count: sections.terms.rows.length },
  ...
  return (
    <Card className={cn("border-[var(--border-strong)]", className)}>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium">Purchase Order Review</CardTitle>
      </CardHeader>
