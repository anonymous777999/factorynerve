...
function getGRNSections(data: OcrPreviewResult) {
  ...
  headers.forEach((header, index) => {
    ...
    } else if (/item|product|description|qty|quantity|rate|amount|price|hsn|sac|unit|uom|received|accepted|rejected|ordered/i.test(h)) {
  ...
  return {
    ...
    qc: {
      headers: qcIndices.map(i => headers[i]),
      rows: rows.map(row => qcIndices.map(i => row[i])),
      colIndices: qcIndices,
    },
  };
}
...
export function GRNReviewView({
  ...
  const tabs = [
    ...
    { id: "items", label: "Items Received", count: sections.items.rows.length },
    { id: "qc", label: "Quality Check", count: sections.qc.rows.length },
  ...
  return (
    <Card className={cn("border-[var(--border-strong)]", className)}>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium">GRN Review</CardTitle>
      </CardHeader>
