...
// A section is a vertical slice of the OCR columns, grouped by meaning.
// `colIndices` maps each local column back to its ORIGINAL position so edits
// and the server's column-type analysis line up.
export type ReviewSection = {
  id: string;
  label: string;
  role: "header" | "table" | "totals";
  headers: string[];
  rows: (string | OcrCell)[][];
  colIndices: number[];
};
...
// A quiet dot shown only when a value needs a second look.
export function AttentionDot({ confidence }: { confidence?: number | null }) {
...
// ── READ MODE: header section as labelled fields (top-of-document feel) ───────
function HeaderBlock({ section, data }: { section: ReviewSection; data: OcrPreviewResult }) {
  if (section.headers.length === 0) return null;
  const row = section.rows[0] ?? [];
  const anyValue = section.headers.some((_h, i) => stringifyOcrCell(row[i]).trim());
...
// ── READ MODE: a formatted, read-only table for line items / details ─────────
function ReadTable({ section, data }: { section: ReviewSection; data: OcrPreviewResult }) {
...
function TotalsPanel({ section, data }: { section: ReviewSection; data: OcrPreviewResult }) {
...
// ── The reusable shell: View/Edit toggle over a set of sections ──────────────
// Read mode renders header sections as labelled blocks, table sections as
// formatted read-only tables, and totals sections as a prominent panel.
// Edit mode preserves the familiar editable grid, one tab per section.
export function SectionedReviewShell({
  title,
  data,
  sections,
  onCellChange,
  ...
  const editTabs = useMemo(
    () => sections.filter((s) => s.rows.length > 0 || s.role === "header"),
    [sections]
  );
  const [activeTab, setActiveTab] = useState(editTabs[0]?.id ?? "");

  const activeId = editTabs.some((t) => t.id === activeTab) ? activeTab : editTabs[0]?.id ?? "";
          ...
          <>
            {sections.map((section) => {
              if (section.role === "header") {
                return <HeaderBlock key={section.id} section={section} data={data} />;
              }
              if (section.role === "totals") {
                if (section.rows.length === 0) return null;
                return (
                  <div key={section.id} className="sm:ml-auto sm:max-w-sm">
                    <TotalsPanel section={section} data={data} />
                  </div>
                );
              }
              if (section.rows.length === 0) return null;
              return (
                <div key={section.id} className="space-y-2">
                  ...
                  <ReadTable section={section} data={data} />
                </div>
              );
            })}
          </>
        ) : (
          <Tabs value={activeId} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value={activeId} className="p-4">
              {editTabs.map((tab) =>
                tab.id === activeId ? (
                  <EditableSection
                    key={tab.id}
                    ...
                    showRowNumbers={tab.role !== "totals"}
                  />
                ) : null
              )}
            </TabsContent>
...
// Helper: build a ReviewSection from original column indices.
