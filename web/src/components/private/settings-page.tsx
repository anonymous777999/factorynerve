...
import { SettingsTabKey } from "@/components/private/settings-tab-nav";
import { SettingsUsageTab } from "@/components/private/settings-usage-tab";
import { SettingsUsersTab } from "@/components/private/settings-users-tab";
            ...
            <SettingsUsersTab
              ...
              onInviteUser={() =>
                void handleAction(async () => {
                  ...
                  setStatus(
                    `${result.user_code ? `User #${result.user_code} ` : "User "}${result.message}. Invitation sent to ${inviteEmail}.`,
                  );
                  setInviteName("");
                  setInviteEmail("");
                  await loadAll();
              ...
              onDeactivateUser={() =>
                void handleAction(async () => {
                  if (!window.confirm(`Are you sure you want to deactivate user ID ${deactivateUserId}? This will remove their access to the platform.`)) {
                    return;
                  }
