...
import { getMyAttendanceToday } from "@/lib/attendance";
...
const STEP_DEFINITIONS = [
  ...
  { title: "Production", caption: "Target, output, manpower, issues, and traceability" },
] as const;
...
export default function EntryPage() {
  ...
  useEffect(() => {
    let alive = true;
    Promise.allSettled([getMe(), getActiveWorkflowTemplate(), getMyAttendanceToday()])
      .then(([userResult, templateResult, attendanceResult]) => {
        if (!alive) return;
        ...
        if (attendanceResult.status === "fulfilled") {
          setAttendanceToday(attendanceResult.value);
        }
      })
      .finally(() => {
  ...
  const alerts = useMemo(() => {
    ...
    if (form.downtime_minutes > 30 && !form.quality_issues) {
      next.push(`${form.downtime_minutes} minutes of downtime recorded without a quality flag. Was there a quality issue?`);
    }
    return next;
  }, [
    conflictId,
  ...
  const notPunchedIn = attendanceToday && !attendanceToday.can_punch_in && attendanceToday.status !== "working" && attendanceToday.status !== "completed";
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_400px]">
          <section className="rounded-[32px] border border-white/10 bg-[linear-gradient(180deg,rgba(21,28,44,0.92),rgba(11,15,25,0.98))] p-5 shadow-[0_24px_80px_rgba(6,10,18,0.48)] backdrop-blur md:p-7">
            ...
            <div className="mt-6 space-y-4">
              ...
              {attendanceToday && attendanceToday.can_punch_in ? (
                <div className="rounded-[24px] border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-100 flex items-center gap-3">
                  <span className="text-lg">⏰</span>
                  <span>
                    You can still punch in for today.{' '}
                    <Link href="/attendance" className="underline underline-offset-4 font-semibold">
                      Punch in
                    </Link>
                    {' '}to record your attendance while filling the entry.
                  </span>
                </div>
              ) : null}
            </div>
            ...
            <div className="mt-6 rounded-[28px] border border-white/10 bg-[#0E1524]/90 p-5 md:p-6">
                  ...
                  <div>
                    ...
                    <div className="mt-3 grid gap-3 sm:grid-cols-3">
                      {SHIFT_OPTIONS.map((option) => {
                        ...
                        const submittedId = shiftMap[option.value];
                        return (
                          <button
                            key={option.value}
                            type="button"
                          ...
                          >
                            ...
                            {submittedId ? (
                              <div className="mt-1 text-xs text-emerald-300/70">Entry #{submittedId} submitted</div>
                            ) : null}
                          </button>
                        );
                      })}
              ...
              {activeStep === 1 ? (
                <div className="space-y-6">
                  {/* Always-visible production fields */}
                  ...
                  {/* Collapsible Issues & Quality section */}
                  <details
                    className="rounded-[24px] border border-white/10 bg-[#0E1524]/70 p-4"
                    open={showIssues || form.downtime_minutes > 0 || form.quality_issues}
                  >
                    <summary
                      className="cursor-pointer list-none text-sm font-semibold text-slate-200 marker:hidden"
                      onClick={(e) => {
                        e.preventDefault();
                        setShowIssues((v) => !v);
                      }}
                    >
                      <span className="flex items-center gap-2">
                        <span>{showIssues || form.downtime_minutes > 0 || form.quality_issues ? "⏷" : "⏵"}</span>
                        <span>Issues &amp; Quality</span>
                        <span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-slate-400">Optional</span>
                      </span>
                    </summary>
                    <div className="mt-4 space-y-5">
                      ...
                      {form.downtime_minutes > 30 && !form.quality_issues ? (
                        <div className="rounded-[20px] border border-amber-400/15 bg-amber-400/10 px-4 py-3 text-sm text-amber-100">
                          ⚠️ {form.downtime_minutes} minutes of downtime recorded. If a quality issue occurred, please turn the quality flag ON above.
                        </div>
                      ) : null}
                    </div>
                  </details>

                  {/* Collapsible Advanced & Traceability section */}
                  <details
                    className="rounded-[24px] border border-white/10 bg-[#0E1524]/70 p-4"
                    open={showAdvanced}
                  >
                    ...
                    >
                      <span className="flex items-center gap-2">
                        ...
                        <span>Advanced &amp; Traceability</span>
                        <span className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-slate-400">Optional</span>
                      </span>
                    </summary>
                    <div className="mt-4 space-y-5">
